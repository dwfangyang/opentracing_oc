...
extern NSString *const YErrorDomain;

/// LightStep error that represents background task failures.
extern NSInteger const YBackgroundTaskError;
...
/// mechanism.
///
/// YTracer is thread-safe.
@interface YTracer : NSObject<OTTracer>

#pragma mark - YTracer initialization

/// @returns An `YTracer` instance that's ready to create spans and logs.
...
/// Initialize an YTracer instance. Either pass the resulting YTracer* around your application explicitly
/// or use the OTGlobal singleton mechanism.
/// Whether calling `-[YTracer flush]` manually or whether using automatic background flushing, users may
...
/// `YTracer` instance's globally unique id ("guid"), and assigned automatically by LightStep.
...
/// The `YTracer` instance's maximum number of records to buffer between reports.
