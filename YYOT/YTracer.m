...
//        _runtimeGuid = [YUtil generateGUID];
        _maxSpanRecords = YDefaultMaxBufferedSpans;
        ...
        _flushTimer = nil;
        _enabled = true;
        _clockState = [[YClockState alloc] init];
...
//    YSpanContext *parent = nil;
    ...
    // No locking required
    return [[YSpan alloc] initWithTracer:self operationName:operationName references:refs tags:tags startTime:startTime];
//    return nil;
}
...
- (BOOL)inject:(id<OTSpanContext>)spanContext
    ...
    YSpanContext *ctx = (YSpanContext *)spanContext;
...
- (id<OTSpanContext>)extractWithFormat:(NSString *)format
    ...
    if ([format isEqualToString:OTFormatTextMap]) {
        ...
        for (NSString *key in dict) {
            if ([key hasPrefix:kBasicTracerBaggagePrefix]) {
                ...
                if ([key isEqualToString:kTraceIdKey]) {
                    foundRequiredFields++;
                    traceId = [dict objectForKey:key];//[YUtil guidFromHex:[dict objectForKey:key]];
                    if (traceId == 0) {
                        if (outError != nil) {
                            *outError =
                    ...
                    foundRequiredFields++;
                    spanId = [dict objectForKey:key];//[YUtil guidFromHex:[dict objectForKey:key]];
                    if (spanId == 0) {
                        if (outError != nil) {
                            *outError =
                                [NSError errorWithDomain:OTErrorDomain code:OTSpanContextCorruptedCode userInfo:nil];
        ...
        return [[YSpanContext alloc] initWithTraceId:traceId spanId:spanId baggage:baggage];
    } else if ([format isEqualToString:OTFormatBinary]) {
        if (outError != nil) {
            *outError = [NSError errorWithDomain:OTErrorDomain code:OTUnsupportedFormatCode userInfo:nil];
        }
...
- (void)flush:(void (^)(NSError *_Nullable error))doneCallback {
    ...
    @synchronized(self) {
        ...
        self.bgTaskId = [[UIApplication sharedApplication]
            beginBackgroundTaskWithName:@"com.lightstep.flush"
                      expirationHandler:^{
                          cleanupBlock(true,
                                       [NSError errorWithDomain:YErrorDomain code:YBackgroundTaskError userInfo:nil]);
                      }];
        if (self.bgTaskId == UIBackgroundTaskInvalid) {
            NSLog(@"unable to enter the background, so skipping flush");
            cleanupBlock(false, [NSError errorWithDomain:YErrorDomain code:YBackgroundTaskError userInfo:nil]);
            return;
        }
    }

    NSString *reqBody = [YUtil objectToJSONString:reqJSON maxLength:LSMaxRequestSize];
    if (reqBody == nil) {
        cleanupBlock(true, [NSError errorWithDomain:YErrorDomain code:LSRequestTooLargeError userInfo:nil]);
        return;
    }
    ...
    SInt64 originMicros = [YClockState nowMicros];
    NSURLSessionDataTask *postDataTask =
        [self.urlSession dataTaskWithRequest:request
            ...
            SInt64 destinationMicros = [YClockState nowMicros];
            NSError *jsonError;
